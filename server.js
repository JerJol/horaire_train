...
console.log('API Key:', NAVITIA_TOKEN ? 'OUI' : 'NON');
...
app.get('/api/trains', async (req, res) => {
    try {
        ...
        if ((!journeysData.journeys || journeysData.journeys.length === 0) && returnJourneys.length === 0) {
            return res.json({ trains: [], returnTrains: [], offset: trainOffset, totalTrains: 0 });
        }
        ...
        const trains = allJourneys.slice(startIdx, startIdx + trainsPerPage).map((journey, idx) => {
            ...
            const trainCode = publicTransportSections.map(sec => 
                sec.display_informations?.headsign || sec.display_informations?.trip_short_name || sec.display_informations?.code || ''
